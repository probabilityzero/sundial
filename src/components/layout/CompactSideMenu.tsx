    ...
    <div className={`fixed inset-y-0 left-0 transform ${isCompact ? 'translate-x-0' : '-translate-x-full'} bg-white shadow-lg transition-transform duration-300 ease-in-out z-50 flex flex-col w-14`}>
      <div className="flex-grow flex flex-col p-1 pt-4">
        ...
        <nav className="flex-grow space-y-0.5 mt-2">
          <Link to="/" style={menuItemStyle} className={`flex justify-center rounded-md transition-colors hover:bg-gray-50 block ${location.pathname === '/' ? 'bg-gray-100' : ''}`}>
            <Home className="w-5 h-5" />
          </Link>
          <Link to="/calendar" style={menuItemStyle} className={`flex justify-center rounded-md transition-colors hover:bg-gray-50 block ${location.pathname === '/calendar' ? 'bg-gray-100' : ''}`}>
            <Calendar className="w-5 h-5" />
          </Link>
          <Link to="/analytics" style={menuItemStyle} className={`flex justify-center rounded-md transition-colors hover:bg-gray-50 block ${location.pathname === '/analytics' ? 'bg-gray-100' : ''}`}>
            <BarChart className="w-5 h-5" />
          </Link>
          <Link to="/tasks" style={menuItemStyle} className={`flex justify-center rounded-md transition-colors hover:bg-gray-50 block ${location.pathname === '/tasks' ? 'bg-gray-100' : ''}`}>
            <ListChecks className="w-5 h-5" />
          </Link>
          <div className="my-1"></div> {/* Added spacing here */}
          <Link to="/settings" style={menuItemStyle} className={`flex justify-center rounded-md transition-colors hover:bg-gray-50 block ${location.pathname === '/settings' ? 'bg-gray-100' : ''}`}>
